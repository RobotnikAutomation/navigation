...
bool AMCLOdom::UpdateNoise(double *noise, AMCLSensorData *data)
{

  AMCLOdomData *ndata;
  ndata = (AMCLOdomData*) data;

  // pf_vector_t is a 3D dimensional vector, really not related to the particle filter
  pf_vector_t old_pose = pf_vector_sub(ndata->pose, ndata->delta);
  ...
  case ODOM_MODEL_OMNI:
  {
    ...
    delta_strafe_hat = 0 + pf_ran_gaussian(strafe_hat_stddev);
    // Apply sampled update to particle pose
    noise[0] += (delta_trans_hat * cs_bearing + 
            delta_strafe_hat * sn_bearing);
    noise[1] += (delta_trans_hat * sn_bearing - 
            delta_strafe_hat * cs_bearing);
    noise[2] += delta_rot_hat; 
  }
  break;
//  case ODOM_MODEL_DIFF:
//  {
...
//
//    // Apply sampled update to particle pose
//    noise[0] += delta_trans_hat * 
//            cos(sample->pose.v[2] + delta_rot1_hat);
//    noise[1] += delta_trans_hat * 
//            sin(sample->pose.v[2] + delta_rot1_hat);
//    noise[2] += delta_rot1_hat + delta_rot2_hat;
//    
//  }
//  break;
//  case ODOM_MODEL_OMNI_CORRECTED:
...
//  break;
  case ODOM_MODEL_DIFF_CORRECTED:
  {
    ...
    delta_rot1_hat = angle_diff(atan2(ndata->delta.v[1], ndata->delta.v[0]),
                              old_pose.v[2]);
